...
const CutoffTable: React.FC<CutoffTableProps> = React.memo(
  ({ data, collegeId }) => {
    ...
    const handleFiltersChange = useCallback(
      (examId: number) => (filters: Record<string, string | null>) => {
        setExamData((prev) => ({
          ...prev,
    ...
    const setExamsWiseCutoff = useCallback(
      (
        setter: SetStateAction<
          Record<number, Record<string, Record<number, number | null>>>
        >
      ) => {
        setExamData((prev) => ({
          ...prev,
    ...
    const handleViewMore = useCallback(
      async (examId: number) => {
        ...
        try {
          const response = await getCollegeFilteredCutoffs(
            collegeId,
            queryParams.toString()
          );
          const newCutoffs = response.cutoffs.reduce(
            (acc: any, cutoff: any) => ({
              ...acc,
          ...
          setExamData((prev) => ({
            ...
            hasMore: {
              ...prev.hasMore,
              [examId]: response.cutoffs.length >= 5,
            },
          }));
        } catch (error) {
    ...
    if (!data?.length)
      return <p className="text-gray-600">No exam data available.</p>;
    ...
    return (
      <div className="space-y-8">
        {data.map((exam) => {
          ...
          const visibleCutoffs = isExpanded
            ? cutoffEntries
            : cutoffEntries.slice(0, ROWS_PER_PAGE);
            ...
            >
              ...
              <div className="p-6">
                <CutoffFilterSection
                  ...
                  setExamsWiseCutoff={
                    setExamsWiseCutoff as Dispatch<
                      SetStateAction<
                        Record<
                          number,
                ...
                {courseCount === 0 ? (
                  <p className="text-center py-8 text-gray-500">
                    <span className="text-red-500 text-lg">✗</span> No data
                    found. Adjust filters to see results.
                  </p>
                ) : (
                  <div className="overflow-x-auto mt-6">
                    ...
                    {courseCount > ROWS_PER_PAGE && (
                      <button
                        ...
                        className="mt-6 ml-auto flex items-center gap-2 bg-black text-white text-xsm font-medium font-public px-2.5 py-0.5 rounded-full hover:bg-primary-main transition-all duration-200"
                      >
